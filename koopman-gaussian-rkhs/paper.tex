...
% $\text{s}$ keeps the plural "s" lowercase: amsart upper-cases titles and running heads except for math.
\title[Koopman invariance of Gaussian RKHS$\text{s}$]{Koopman invariance of Gaussian RKHS$\text{s}$ for linear
stochastic differential equations: a necessary and sufficient condition}

% Author data to be completed by the submitting author:
% \author{First Last}
% \address{Institution, Address}
% \email{name@example.org}
% Before submission, also activate and adapt the AI-use statement before the bibliography
% (see "Use of AI tools" below); most journals require such a statement.
...
Gaussian kernel $\exp(-(x-y)^\tp C^{-2}(x-y))$ is invariant under the Koopman semigroup of the linear stochastic
...
$e^{At}C^2e^{A^\tp t}\le C^2+4\Sigma(t)$, and then $\|K^t\|=e^{-t\Tr A/2}$. For Hurwitz $A$, invariance for all
$t\ge0$ holds if and only if the ellipsoid of $C^2+4\Sigma$, where $\Sigma$ is the stationary covariance, is forward
invariant under $\dot x=Ax$. The proofs rest on a Fourier description of Gaussian RKHSs, in which $K^t$ becomes a weighted
...
The Koopman operator was introduced in \cite{Koopman} for deterministic systems. For a stochastic dynamical
system $(X_t)_{t\ge0}$ it acts on observables $f$ by
\[
(K^tf)(x)=\mathbb E[f(X_t)\,|\,X_0=x].
\]
It is linear even when the dynamics are not, and it can be approximated from data, for instance by extended dynamic mode decomposition (EDMD) \cite{WilliamsEDMD} and its kernel-based
variants \cite{Klus,WilliamsKernel}; see \cite{Mauroy} for an overview. As pointed out in \cite{PSWPN}, the derivation
of error bounds for kernel-based approximations of Koopman operators \cite{Hertel,Koehne,Kostic,PhilippACHA} benefits greatly
...
Gaussian kernel
\[
k^C(x,y)=\exp\bigl(-(x-y)^\tp C^{-2}(x-y)\bigr)=\exp\bigl(-|C^{-1}(x-y)|^2\bigr),\quad x,y\in\R^d ,
\]
with $C\in\R^{d\times d}$ symmetric positive definite. Let $H_C$ be the (real) RKHS of $k^C$ on $\R^d$, with norm
$\|\cdot\|_C$, and let
...
Here $K^tf$ is defined pointwise by $(K^tf)(x)=\mathbb E[f(X_t)\,|\,X_0=x]$, which makes sense because every
function in $H_C$ is bounded and continuous (Section~\ref{sec:koopman}). Under the standing assumptions of \cite{PSWPN} this agrees with their definition on
...
Gaussian RKHSs are weighted $L^2$-spaces in Fourier variables: a continuous real-valued $f\in L^2(\R^d)$ belongs
to $H_C$ if and only if $\int|\hatf f(\xi)|^2e^{\frac14\xi^\tp C^2\xi}\dd\xi<\infty$ (Lemma~\ref{lem:fourier}). In Fourier variables, $K^t$
...
$\tau_t>0$, and consequently $K^t$ maps $H_C$ into $H_{C_t}$ \cite[Proposition~4.1 and its proof]{PSWPN}. Moreover,
\eqref{eq:23} is equivalent to $H_{C_t}\subset H_C$ for all $t\ge0$ (see the proof of \cite[Theorem~2.1]{PSWPN}). Membership in $H_C$, however, is decided by $|\hatf f|^2$. In
...
$4\Sigma(t)$ and $BB^\tp$ by $2BB^\tp$. For small $t$, finite kernel expansions cannot detect non-invariance
(Remark~\ref{rem:sections}). The witnesses of non-invariance constructed below have Fourier transforms that decay,
within a cone of directions, as slowly as membership in $H_C$ permits (Lemma~\ref{lem:multiplier}).
...
The same Fourier argument works for every translation-invariant kernel whose spectral density is positive and
integrable (Proposition~\ref{prop:general}). For instance, Sobolev (Mat\'ern) RKHSs are invariant for all $A$ and $B$
...
w_C(\xi):=\exp\bigl(\tfrac14\xi^\tp C^2\xi\bigr)\quad(\xi\in\R^d),\qquad \kappa_C:=\pi^{d/2}\det C ,
\end{equation}
and we let $\FC{C}$ be the set of all measurable Hermitian functions $F\colon\R^d\to\mathbb C$ with
$\int|F|^2w_C\dd\xi<\infty$. By \eqref{eq:gauss} with $P=C^{-2}$, followed by a translation,
...
\[
\|F\|_{L^1}\le\Bigl(\frac{(4\pi)^{d/2}}{\det C}\Bigr)^{1/2}\Bigl(\int|F|^2w_C\dd\xi\Bigr)^{1/2}.
\]
...
\[
\int|F|\dd\xi=\int|F|w_C^{1/2}\,w_C^{-1/2}\dd\xi
\le\Bigl(\int|F|^2w_C\dd\xi\Bigr)^{1/2}\Bigl(\int w_C^{-1}\dd\xi\Bigr)^{1/2}.
\]
Since $w_C\ge1$, also $\int|F|^2\dd\xi\le\int|F|^2w_C\dd\xi$.
...
\begin{align*}
\ip f{k^C_y}_{\mathcal H}
&=(2\pi)^{-d}\kappa_C^{-1}\int\hatf f(\xi)\,\kappa_C e^{i\xi\cdot y}w_C(\xi)^{-1}w_C(\xi)\dd\xi\\
...
\begin{align*}
\int|F_S|^2w_C\,e^{\frac14\xi^\tp S\xi}\dd\xi
&\ge\int_\Gamma(1+|\xi|^2)^{-d}e^{\lambda|\xi|^2/8}\dd\xi\\
...
\end{equation}
Every $f\in H_C$ is continuous by Lemma~\ref{lem:fourier}(b), and bounded, since
...
For $f\in H_C$ and $t\ge0$ the following identity holds in $[0,\infty]$:
\begin{equation}\label{eq:normid}
\int_{\R^d}\bigl|\hatf{K^tf}(\xi)\bigr|^2w_C(\xi)\dd\xi
=e^{-t\Tr A}\int_{\R^d}|\hatf f(\eta)|^2\,w_C(\eta)\,e^{-\frac14\eta^\tp D_t\eta}\dd\eta .
\end{equation}
\end{proposition}
...
\[
e^{-2t\Tr A}\int\bigl|\hatf f(e^{-A^\tp t}\xi)\bigr|^2
\exp\bigl(-\xi^\tp e^{-At}\Sigma(t)e^{-A^\tp t}\xi+\tfrac14\xi^\tp C^2\xi\bigr)\dd\xi .
\]
The substitution $\xi=e^{A^\tp t}\eta$, $\mathrm d\xi=e^{t\Tr A}\mathrm d\eta$, turns this into
\[
...
\emph{Sufficiency in (a) and the upper bound in (b).} If $D_t\ge0$, then the factor $e^{-\frac14\eta^\tp D_t\eta}$ in
\eqref{eq:normid} is at most $1$. Hence $K^tf\in H_C$ and, by \eqref{eq:normC}, $\|K^tf\|_C^2\le e^{-t\Tr A}\|f\|_C^2$.
...
\begin{align*}
\|K^tf_\varepsilon\|_C^2
&=e^{-t\Tr A}(2\pi)^{-d}\kappa_C^{-1}\int_{|\eta|<\varepsilon}w_C(\eta)e^{-\frac14\eta^\tp D_t\eta}\dd\eta\\
...
\begin{align*}
\int|\hatf g(\xi)|^2w_{\Ct}(\xi)\dd\xi
&=e^{-t\Tr A}\int|\hatf f(\eta)|^2
e^{-\eta^\tp\Sigma(t)\eta+\frac14\eta^\tp e^{At}\Ct^{\,2}e^{A^\tp t}\eta}\dd\eta\\
&=e^{-t\Tr A}\int|\hatf f(\eta)|^2w_C(\eta)\dd\eta ,
\end{align*}
...
\[
F(\eta):=e^{t\Tr A}\,\hatf g(e^{A^\tp t}\eta)\exp\bigl(\tfrac12\eta^\tp\Sigma(t)\eta\bigr),\qquad\eta\in\R^d .
\]
Then $F$ is Hermitian. With $\eta=e^{-A^\tp t}\xi$, $\mathrm d\eta=e^{-t\Tr A}\mathrm d\xi$ and
$\eta^\tp\Sigma(t)\eta+\frac14\eta^\tp C^2\eta=\frac14\xi^\tp\Ct^{\,2}\xi$ we get
\begin{align*}
\int|F(\eta)|^2w_C(\eta)\dd\eta
&=e^{2t\Tr A}\int|\hatf g(e^{A^\tp t}\eta)|^2e^{\eta^\tp\Sigma(t)\eta+\frac14\eta^\tp C^2\eta}\dd\eta\\
&=e^{t\Tr A}\int|\hatf g(\xi)|^2w_{\Ct}(\xi)\dd\xi<\infty .
\end{align*}
Hence $F\in\FC{C}$, and $f:=F^\vee\in H_C$ by Lemma~\ref{lem:fourier}(c). By \eqref{eq:KtFT},
\[
\hatf{K^tf}(\xi)=e^{-t\Tr A}F(e^{-A^\tp t}\xi)\exp\bigl(-\tfrac12\xi^\tp e^{-At}\Sigma(t)e^{-A^\tp t}\xi\bigr)=\hatf g(\xi).
\]
Since $K^tf$ and $g$ are continuous $L^2$-functions with the same Fourier transform, $K^tf=g$.
\end{proof}
...
For the last assertion, let $f_x(t):=\ip{C_t^2x}x$ as in the proof of \cite[Theorem~2.1]{PSWPN}. Since $\Sigma(0)=0$
and $\dot\Sigma(0)=BB^\tp$, differentiating $C_t^2=e^{-At}(C^2+2\Sigma(t))e^{-A^\tp t}$ at $t=0$ gives
...
Let $A$ be Hurwitz (controllability is not needed), and let $\Sigma:=\int_0^\infty e^{As}BB^\tp e^{A^\tp s}\dd s$.
Put $P_C:=C^2+4\Sigma$, which is positive definite, and let $\mathcal E_C:=\{x\in\R^d:\ |x|_{P_C}\le1\}$ be the unit
ball of the norm $|x|_{P_C}:=(x^\tp P_C^{-1}x)^{1/2}$.
\begin{enumerate}
\item[(a)] $D_t=P_C-e^{At}P_Ce^{A^\tp t}$ for all $t\ge0$. Consequently, for each $t\ge0$,
\begin{align*}
K^tH_C\subset H_C
&\iff e^{At}P_Ce^{A^\tp t}\le P_C\\
&\iff |e^{At}x|_{P_C}\le|x|_{P_C}\ \text{ for all }x\in\R^d\\
...
\begin{remark}
Since $A\Sigma+\Sigma A^\tp=-BB^\tp\le0$, the ellipsoid of $\Sigma$ is forward invariant whenever $\Sigma>0$
(e.g.\ if $(A,B)$ is controllable). By
...
Let $d=m=2$, $B=C=I_2$, and
\[
A=A_\beta:=\begin{pmatrix}-1&\beta\\0&-1\end{pmatrix},\qquad\beta\ge0 .
\]
Then $A_\beta$ is Hurwitz (double eigenvalue $-1$), and $(A_\beta,B)$ is controllable since $B$ is invertible. The
matrix $AC^2+C^2A^\tp=A_\beta+A_\beta^\tp$ has eigenvalues $-2\pm\beta$, so
\begin{align*}
\eqref{eq:23}&\iff\tfrac12(\beta-2)\le1\iff\beta\le4,\\
...
Everything here is explicit. We have $A_\beta=-I+\beta E_{12}$, where $E_{12}$ has the entry $1$ in position $(1,2)$
and zeros elsewhere. Since $E_{12}^2=0$,
\[
e^{A_\beta t}=e^{-t}\begin{pmatrix}1&\beta t\\0&1\end{pmatrix},\qquad
\Sigma(t)=\int_0^te^{-2s}\begin{pmatrix}1+\beta^2s^2&\beta s\\\beta s&1\end{pmatrix}\dd s .
\]
The elementary integrals
\begin{gather*}
\int_0^te^{-2s}\dd s=\frac{1-e^{-2t}}2,\qquad\int_0^tse^{-2s}\dd s=\frac{1-e^{-2t}(1+2t)}4,\\
\int_0^ts^2e^{-2s}\dd s=\frac{1-e^{-2t}(1+2t+2t^2)}4
\end{gather*}
give the entries of $D_t=(d_{ij}(t))_{i,j=1}^2$:
\begin{align*}
d_{11}(t)&=3(1-e^{-2t})+\beta^2\bigl(1-e^{-2t}(1+2t+3t^2)\bigr),\\
d_{12}(t)&=d_{21}(t)=\beta\bigl(1-e^{-2t}(1+3t)\bigr),\qquad d_{22}(t)=3(1-e^{-2t}).
\end{align*}
In particular, in accordance with \eqref{eq:Dint},
\[
D_t=t\,\Lambda_C+O(t^2)\quad\text{as }t\downarrow0,\qquad
\Lambda_C=4I-(A_\beta+A_\beta^\tp)=\begin{pmatrix}6&-\beta\\-\beta&6\end{pmatrix}.
\]
Furthermore,
\[
\Sigma=\begin{pmatrix}\frac12+\frac{\beta^2}4&\frac\beta4\\[2pt]\frac\beta4&\frac12\end{pmatrix},\qquad
P_C=I+4\Sigma=\begin{pmatrix}3+\beta^2&\beta\\\beta&3\end{pmatrix},
\]
and
\[
...
Let $d=2$, $m=1$, $C=I_2$, and
\[
A=\begin{pmatrix}\frac32&-2\\[2pt]2&-2\end{pmatrix},\qquad B=\begin{pmatrix}1\\0\end{pmatrix}.
\]
Then $\Tr A=-\frac12$ and $\det A=1$, so the eigenvalues $-\frac14\pm\frac{\sqrt{15}}4i$ have negative real parts and
$A$ is Hurwitz. Moreover $AB=(\frac32,2)^\tp$, so $\det[B,AB]=2\neq0$ and $(A,B)$ is controllable. Since
$\frac12(A+A^\tp)=\operatorname{diag}(\frac32,-2)$ and $BB^\tp=\operatorname{diag}(1,0)$, we have
\begin{align*}
...
\begin{align*}
\ip{k^{C'}_y}{k^{C'}_{y'}}_{C''}
&=(2\pi)^{-d}\frac{\kappa_{C'}^2}{\kappa_{C''}}\int e^{-\frac14\xi^\tp G\xi-i\xi\cdot(y-y')}\dd\xi\\
...
By \eqref{eq:KtFT} and \eqref{eq:kernelFT},
\[
\hatf{K^tk^C_z}(\xi)=e^{-t\Tr A}\kappa_C\,e^{-i\xi\cdot e^{-At}z}\,w_{C_t}(\xi)^{-1},\qquad\xi\in\R^d ,
\]
and since $\det C_t=e^{-t\Tr A}\det(C^2+2\Sigma(t))^{1/2}$, this means $K^tk^C_z=\tau_t\,k^{C_t}_{e^{-At}z}$, as in the
proof of \cite[Proposition~4.1]{PSWPN}. By Lemma~\ref{lem:bumps}, $K^tk^C_z\in H_C$ if and only if $2C_t^2-C^2>0$. Now
$2C_t^2-C^2=e^{-At}(C^2+D_t)e^{-A^\tp t}$ and $D_t\to0$ as $t\downarrow0$. Hence, for all sufficiently small $t$, $K^t$
maps $\operatorname{span}\{k^C_z:z\in\R^d\}$ into $H_C$ \emph{regardless} of the sign of $D_t$. Indeed, a finite kernel
expansion $f$ satisfies $|\hatf f|^2w_C\le c\,w_C^{-1}$ for some $c\ge0$ by \eqref{eq:kernelFT}, so the right-hand side
of \eqref{eq:normid} is finite whenever $C^2+D_t>0$. Witnesses of non-invariance must therefore have Fourier
transforms that decay more slowly; those of Lemma~\ref{lem:multiplier} decay, within a cone of directions, as slowly as
membership in $H_C$ permits. This explains why the necessary condition
...
Theorem~\ref{thm:range} states that
\[
\ip{K^tk^C_z}{K^tk^C_{z'}}_{\Ct}=\taut\,k^C(z,z'),\qquad z,z'\in\R^d .
\]
This identity also follows directly from $K^tk^C_z=\tau_t\,k^{C_t}_{e^{-At}z}$ (Remark~\ref{rem:sections}) and
Lemma~\ref{lem:bumps}, because
$2C_t^2-\Ct^{\,2}=e^{-At}C^2e^{-A^\tp t}$.
\end{remark}
...
Ikeda, Ishikawa and Sawano \cite{IIS} proved that on a large class of RKHSs of analytic positive definite functions
only affine maps induce bounded composition operators. For the Fock space of entire functions, bounded composition operators were characterized in
...
\]
Then $K^tH_\varphi\subset H_\varphi$ if and only if $M_t:=\sup_\eta m_t(\eta)$ is finite, and in this case
\[
\|K^t\|_{H_\varphi\to H_\varphi}^2=e^{-t\Tr A}M_t .
\]
For the Gaussian kernel, i.e.\ for $\varphi(z)=e^{-z^\tp C^{-2}z}$, we have $m_t(\eta)=e^{-\frac14\eta^\tp D_t\eta}$;
this recovers Theorem~\ref{thm:fixed}(a),(b).
\end{proposition}
...
\[
\|K^tf\|_{H_\varphi}^2=(2\pi)^{-d}e^{-t\Tr A}\int|\hatf f(\eta)|^2\,\frac{m_t(\eta)}{\hatf\varphi(\eta)}\dd\eta
\]
whenever the right-hand side is finite, and $K^tf\notin H_\varphi$ otherwise. If $M_t<\infty$, then
$\|K^tf\|_{H_\varphi}^2\le e^{-t\Tr A}M_t\|f\|_{H_\varphi}^2$. For the reverse inequality, let $\delta>0$ and choose
$\eta_0$ with $m_t(\eta_0)>M_t-\delta$. The function $m_t$ is continuous and even. Hence, for small $r>0$, the
function $f_r:=(\mathbf 1_{U_r})^\vee$ with $U_r:=\{|\eta-\eta_0|<r\}\cup\{|\eta+\eta_0|<r\}$ belongs to
$H_\varphi\setminus\{0\}$ and satisfies $\|K^tf_r\|^2\ge e^{-t\Tr A}(M_t-\delta)\|f_r\|^2$.

Suppose now that $M_t=\infty$. For $k\in\mathbb N$ the set $\{m_t>16^k\}$ is open, nonempty and symmetric. Choose a
...
$\|K^t\|\le e^{-t\Tr A/2}\max\{1,\|e^{At}\|\}^{s}$. The restriction \eqref{eq:23sharp} therefore comes from the
Gaussian decay of the spectral density: for the Gaussian kernel the ratio $\hatf\varphi/(\hatf\varphi\circ e^{A^\tp t})$
in $m_t$ is the exponential of a quadratic form, which competes with the Gaussian factor
$\exp(-\eta^\tp\Sigma(t)\eta)$. Koopman invariance of the native spaces of Wendland kernels, which are norm-equivalent to
Sobolev spaces, is proved in \cite{Koehne}; stochastic dynamics are treated in \cite{Hertel}.
\end{example}
...
The identities above were also checked with three Python scripts, which are provided as supplementary material:
two in floating-point arithmetic (NumPy, SciPy and Matplotlib; random test data from a fixed seed) and one in exact
arithmetic (SymPy). No proof in this paper depends on these checks. Matrix errors are measured in the entrywise
norm $|M|_{\max}:=\max_{i,j}|M_{ij}|$. The covariance $\Sigma(t)$ was computed with Van Loan's block-exponential
formula \cite{VanLoan} on steps of length at most $0.25$, combined with the recursion
$\Sigma(s+h)=\Sigma(h)+e^{Ah}\Sigma(s)e^{A^\tp h}$. The one-step formula cancels catastrophically for large $t$.
\begin{enumerate}
\item $\Sigma(t)$ agrees with its value $\Sigma_{\mathrm q}(t)$ from adaptive quadrature:
$|\Sigma(t)-\Sigma_{\mathrm q}(t)|_{\max}\le2.7\times10^{-13}\,|\Sigma_{\mathrm q}(t)|_{\max}$ (random $A\in\R^{3\times3}$,
$B\in\R^{3\times2}$, $t\in\{0.3,1.7,4\}$).
\item Let $R_t$ be the right-hand side of \eqref{eq:Dint}, computed by adaptive quadrature. Then
$|D_t-R_t|_{\max}\le5.2\times10^{-13}\max\{1,|R_t|_{\max}\}$ for three random triples $(A,B,C)$ with $A$ \emph{not}
Hurwitz (spectral abscissae $1.645$, $1.556$, $2.043$) and $t\in\{0.05,0.8,2.5\}$. Moreover
$|t^{-1}D_t-\Lambda_C|_{\max}\le3.7\times10^{-6}\max\{1,|\Lambda_C|_{\max}\}$ at $t=10^{-6}$, consistent with the $O(t)$
remainder.
...
\item Theorem~\ref{thm:range} on kernel sections, for random $2\times2$ data and $t\in\{0.2,0.9\}$. We compute the
Gram matrix $\bigl(\ip{K^tk^C_{z_i}}{K^tk^C_{z_j}}_{\Ct}\bigr)_{i,j\le4}$ from Lemma~\ref{lem:bumps} and the formula
$K^tk^C_z=\tau_t\,k^{C_t}_{e^{-At}z}$ of \cite[proof of Proposition~4.1]{PSWPN}. Its entries differ from those of
$\taut\,(k^C(z_i,z_j))_{i,j}$ by at most $3.8\times10^{-14}$. The value
$\|K^tk^C_z\|_C^2=e^{-t\Tr A}\det C\,\det(2C^2+4\Sigma(t)-e^{At}C^2e^{A^\tp t})^{-1/2}$, obtained from
\eqref{eq:normid}, agrees with the value given by that formula and Lemma~\ref{lem:bumps} to a relative error of
$1.1\times10^{-13}$.
\item Monte Carlo check of the formula $(K^tk^C_z)(x)=\tau_t\,k^{C_t}_{e^{-At}z}(x)$ for Example~\ref{ex:beta} with
$\beta=5$, $t=0.4$, $x=(0.3,-0.2)^\tp$ and $z=(0.5,0.4)^\tp$, using $2\times10^6$ exact Gaussian samples of $Z_t$:
$0.436373\pm2.2\times10^{-4}$ (one standard error) against the closed form $0.436051$, a deviation of $1.45$ standard
errors.
...
\item Example~\ref{ex:scalar}: the matrix $A$ has the eigenvalues $-0.25\pm0.968i$, and the controllability matrix
$[B,AB]$ has rank $2$. Moreover $\lambda_{\max}\bigl(\frac12(A+A^\tp)-BB^\tp\bigr)=0.5$ and
$\lambda_{\max}\bigl(\frac12(A+A^\tp)-2BB^\tp\bigr)=-0.5$, and $\lambda_{\min}(D_t)\ge0$ on $3000$ grid points
in $(0,10]$.
...
\end{enumerate}
In total the script for items 1--6 and 8--10 performs $38$ automated checks, all of which pass. The exact-arithmetic
script confirms the closed forms in Examples~\ref{ex:beta} and~\ref{ex:scalar} (the stationary covariance in
Example~\ref{ex:scalar} is $\Sigma=\bigl(\begin{smallmatrix}5&4\\4&4\end{smallmatrix}\bigr)$), the constants in
\eqref{eq:gauss}--\eqref{eq:normC} and Lemma~\ref{lem:bumps}, the bound $\lambda_{\min}(P_C)/\lambda_{\max}(P_C)\ge10^{-2}$
for $\beta\le14$ used in Example~\ref{ex:beta}, and Theorem~\ref{thm:range} for $d=1$ and $f=k^C_z$. It performs $52$
checks, all of which pass.

% ---------------------------------------------------------------------------------------------
% Use of AI tools. Most publishers require this statement (SIAM: Editorial Policy on Artificial
% Intelligence; Elsevier: a separate declaration before the references). Uncomment it, add the
% tool version and dates your journal asks for, and make sure that the last sentence is true.
% \subsection*{Use of AI tools}
% The results, proofs, text, numerical scripts and figure of this paper were developed with
% substantial assistance from the AI assistant Claude (Anthropic), under the direction of the author,
% who checked every argument and takes full responsibility for the content.
% ---------------------------------------------------------------------------------------------

\subsection*{Data availability}
No external data were used. The scripts that produce the numbers in Appendix~\ref{app:numerics} and
Figure~\ref{fig:times} are provided as supplementary material.
...
(PAMM) \textbf{25} (2025), no.~1, e202400127; arXiv:2405.14429. Numbered statements of this paper are cited as in
arXiv:2405.14429v1.
