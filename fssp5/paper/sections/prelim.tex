...
\Cref{fig:moods} shows the states of a five-state rule and one step of one
cell.

\begin{figure}[t]
\centering
\input{sections/pic_moods}
\caption{Left: the five states of a five-state rule, drawn as soldiers.
Right: one step of one cell. The cell in the middle sees its left neighbour
in state $\mathsf G$, itself in state $\mathsf L$ and its right neighbour in
state $\mathsf L$, looks up the entry $\mathsf G\,\mathsf L\,\mathsf L$ of
the rule and becomes $\mathsf A$. The four entries shown belong to the
rule $\delta_{14}$ of \cref{tab:delta14}.}
\label{fig:moods}
\end{figure}
...
It is classical \cite{Moore64,Waksman66,Balzer67} that a rule which
synchronizes \emph{every} line cannot synchronize the line of length $n\ge2$
before time $2n-2$ (see \cref{rem:lowerbound}). A \emph{minimal-time
solution} is a rule that synchronizes the line of length $n$ at time exactly
$2n-2$ for every $n\ge2$. (For $n=1$ one
...
at time $2n-2$ for every $n\in\mathcal N$. A minimal-time solution is
$\mathcal N$-minimal for every $\mathcal N$. Since there are finitely many
rules with $k$ states, $k$-state minimal-time solutions do not exist if and
only if there is a finite $N$ such that no $k$-state rule is
$\{2,\dots,N\}$-minimal; a non-existence proof may therefore consist of a
finite family of necessary conditions that no $k$-state rule satisfies.
This is how we prove the four-state bound (\cref{thm:four}); for five states
we study how large such a family must be.
...
$C_n(t,i)=C_\infty(t,i)=\mathsf L$ (for $C_n$, whenever $i\le n$).
\end{lemma}
...
\]
In particular the \emph{front} $f_i:=C_\infty(i-1,i)$ satisfies
$f_{n-1}\neq\mathsf L$, and $\delta(f_{n-1},\mathsf L,\ast)\neq
\delta(f_{n-1},\mathsf L,\mathsf L)$.
\end{lemma}
...
For $i=n$ we get $C_n(n-1,n)=\delta(f_{n-1},\mathsf L,\ast)\neq
C_\infty(n-1,n)=\delta(f_{n-1},\mathsf L,\mathsf L)=f_n$, which is impossible
if $f_{n-1}=\mathsf L$ by \eqref{eq:quiescence}.
\end{proof}

\begin{remark}\label{rem:lowerbound}
Suppose a rule synchronizes every line, the line of length $n$ at some time
$T\le 2n-3$. By \cref{lem:agree}, $C_\infty(T,1)=C_n(T,1)=\mathsf F$, hence
$C_{n'}(T,1)=\mathsf F$ for every $n'\ge n$. For $n'>T+1$ the cell $n'$ is still
quiescent at time $T$ by \cref{lem:lightcone}, so the line of length $n'$
has a firing cell at time $T$ without firing entirely: a contradiction.
\end{remark}

\Cref{fig:echo} illustrates \cref{lem:agree} and \cref{rem:lowerbound}.
The right border only acts after the front has reached it at time $n-1$,
and its influence travels back at speed one; below the anti-diagonal
$t+i=2n-2$ the line of length $n$ cannot be told apart from any longer
line, and the first cell learns where the line ends exactly at time
$2n-2$.

\begin{figure}[t]
\centering
\input{sections/pic_echo}
\caption{Space-time diagrams (time runs downwards, one square per cell and
time step) of the lines of lengths $6$ and $9$, coloured by what each cell
can know. On the cells with $t+i\le10$ (light blue, bounded by the dashed
anti-diagonal) the two diagrams coincide for every rule (\cref{lem:agree});
the right border acts only in the reflected triangle $R_n$, below the
return chain. The colours show regions, not states.}
\label{fig:echo}
\end{figure}
