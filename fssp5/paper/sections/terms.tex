%% ---------------------------------------------------------------------
\section{Glossary of terms and symbols}\label{app:glossary}

This appendix defines every term and every symbol used in the paper; the
most frequent notation is summarised on the page \emph{Notation at a glance}
before the introduction. The entries are grouped
...
collected in one entry of \cref{app:gl-geometry}, carry no formal content,
and no statement of the paper depends on them. The symbols are summarised in
\cref{tab:symbols} at the end of the appendix.

{\small
\subsection{General conventions}\label{app:gl-general}
...
\item[Typography.] Sans-serif capitals $\mathsf L,\mathsf G,\mathsf
  ...
  propositional variable (\cref{app:gl-logic}). Program names in typewriter font
  (\texttt{fsspcheck.c}, \dots) refer to files of the accompanying
  repository, listed in \ref{app:repro}.
\end{description}

\subsection{The firing squad}\label{app:gl-squad}
\begin{description}[style=unboxed,leftmargin=1em,font=\normalfont\bfseries,itemsep=1pt,parsep=0pt,topsep=2pt]
\item[Cellular automaton (CA).] A row of identical finite automata,
  ...
  \cref{app:gl-geometry}).
\item[Cell; position.] The cells of a line are numbered $1,\dots,n$
  from left to right; the number $i$ of a cell is its \emph{position}. In the figures a cell is drawn as a soldier.
...
\item[Renaming; labelling.] A \emph{renaming} is a bijection $\pi$ of the
  states; applied to a rule or a diagram it replaces every state $x$ by
  $\pi(x)$. Two diagrams coincide up to renaming if one is the image of the
  other; the images of one diagram under different renamings are its
  \emph{labellings}.
...
\item[The rules $\delta_{12}$, $\delta_{13}$, $\delta_{14}$,
  $\delta_{14}^{\mathrm b},\dots,\delta_{14}^{\mathrm e}$.] Explicit
  five-state rules found in this work (\cref{sec:five} and
  \cref{app:more14}). The index is the largest $N$ for which the rule is
  $\{2,\dots,N\}$-minimal ($12$, $13$, and $14$ for the last five); each of
  them fails at length $N+1$.
...
\item[Picture vocabulary.] In \cref{fig:squad,fig:moods,fig:echo,fig:hare,fig:scoreboard}
  a cell is drawn as a \emph{soldier}, a line is a \emph{squad}, a state a
  \emph{mood}, a time step a \emph{tick}, the rule the \emph{rule book}
  and the border a \emph{wall}; the \emph{hare} and the \emph{tortoise} are
  signals of speeds $1$ and $1/3$, and the \emph{echo} is the reflected wave
  (\cref{app:gl-geometry}).
\end{description}

\subsection{Geometry of space-time diagrams}\label{app:gl-geometry}
\begin{description}[style=unboxed,leftmargin=1em,font=\normalfont\bfseries,itemsep=1pt,parsep=0pt,topsep=2pt]
...
\item[Half-line; $C_\infty$.] The cells $1,2,3,\dots$ with the border on
  ...
  in this paper: no cell of $C_\infty$ fires, see \eqref{eq:neverfire}, the
  front is never $\mathsf L$ (\cref{lem:return}), and $\mathrm{PUMP}(n,n')$
  holds (\cref{app:gl-barriers}).
...
\subsection{The two barriers}\label{app:gl-barriers}
\begin{description}[style=unboxed,leftmargin=1em,font=\normalfont\bfseries,itemsep=1pt,parsep=0pt,topsep=2pt]
...
\item[Eventually regular; junction words; blocks.] (Definition before
  ...
  with words $w_j$ and nonempty words $z_j$ over $W$ and integers
  $a_j,D_j\ge0$ that depend on $r$ only. The $w_j$ are the \emph{junction words} (they carry the
...
\item[Regularity certificate.] The finite test of \cref{lem:certificate},
  which proves that a given half-line is eventually
  regular (program \texttt{germpersist.py}).
\end{description}

\subsection{Germs}\label{app:gl-germs}
\begin{description}[style=unboxed,leftmargin=1em,font=\normalfont\bfseries,itemsep=1pt,parsep=0pt,topsep=2pt]
...
\subsection{Propositional logic, solvers and certificates}\label{app:gl-logic}
\begin{description}[style=unboxed,leftmargin=1em,font=\normalfont\bfseries,itemsep=1pt,parsep=0pt,topsep=2pt]
...
\item[Counterexample-guided search over lengths.] Solve for a set of
  ...
  length and repeat (\cref{app:more14}).
...
\item[Look-ahead search tree; random-probe estimator.] The tree of
  ...
  random root-to-leaf paths (\emph{probes}); if $d_1,\dots,d_D$ are the
  numbers of children of the nodes of a probe before its leaf, then
  $1+d_1+d_1d_2+\dots+d_1\cdots d_D$ is an unbiased estimate of the number
  of nodes, and the estimates are averaged over the probes.
\item[Cube-and-conquer.] Splitting a formula into many subproblems, each
  ...
  independently \cite{HeuleEtAl11}; the cubes are produced by the
  look-ahead solver \textsf{march\_cu}, and a cube is \emph{refuted} when
  the formula together with it is unsatisfiable. Together the cubes cover
  all assignments, so the formula is unsatisfiable if and only if every cube
  is refuted.
...
{\small
\begin{longtable}{@{}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.52\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}@{}}
...
$X(\mathbf e)$, $\mathbf 1_j$, $\Lambda$, $q_j$, $\ell_\nu$ & block word, unit vector, $P$ steps of the half-line; local to \cref{lem:certificate} & \cref{lem:certificate}\\
$\bar\imath$ & mirrored position $n+1-i$ & \cref{rem:right}\\
$\mathrm{MT}_k(\mathcal N;M,N')$ & the formula; $M$: half-line bound, $N'$: pumping bound (omitted: no pumping) & \cref{sec:encoding}\\
...
$\delta_{12},\dots,\delta_{14}^{\mathrm e}$ & the five-state rules of \cref{sec:five} and \cref{app:more14} & \cref{sec:five}, \cref{app:more14}\\
\end{longtable}}
