...
minimal-time solution exists; here $M$ bounds the half-line and $N'$ the
pumping constraints, and we write $\mathrm{MT}_k(\mathcal N;M)$ when no
pumping constraints are added. All constraint families below are either the
...
\paragraph{Half-line} The cells $(t,i)$ of $C_\infty$ with $t\ge1$,
$1\le i\le t+1$ and $t+i\le M$ receive variables and transition clauses; cells with
...
$E_{n,n'}(\kappa_0)\Rightarrow\bigl(C_n(n-1+\tau,n-\kappa)=C_{n'}(n'-1+\tau,n'-\kappa)\bigr)$,
...
the encoded half-line cells, $\mathsf A_{j+1}$ occurs only after a first
occurrence of $\mathsf A_{j}$; every solution can be renamed to satisfy
