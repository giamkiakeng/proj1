...
$\mathrm{MT}_6(\{3,\dots,12\};78,40)$ with all options enabled (the
formalization does not cover $n=2$), and with symmetry breaking exactly one
of the $3!$ renamings of its auxiliary states satisfies the formula, as it
must. Conversely, every model found by the solvers was decoded and
re-simulated by an independent brute-force checker (\texttt{fsspcheck.c}),
which confirmed minimal-time synchronization for all encoded lengths.
